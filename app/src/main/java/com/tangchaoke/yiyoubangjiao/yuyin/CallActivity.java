...
//            EMChatManager.getInstance().removeCallStateChangeListener(callStateListener);
    ...
    protected void saveCallRecord(int type) {
        ...
        Log.e("===callingState:::", callingState + "---");
        switch (callingState) {
            case NORMAL:
        ...
        message.setAttribute(EaseConstant.TO_NAME, mToName);//发送 还是发送自己的昵称
        message.setAttribute(EaseConstant.TO_HEAD, mToHead);//发送 还是发送自己的头像
        message.setAttribute(EaseConstant.TO_ID, mToId);//发送 还是发送自己的ID
        message.setAttribute(EaseConstant.PROBLEM_OID, mProblemOid);//发送 问题ID

        Log.e("==视频扩展消息：：：", mToName + "---mToName---"
                + mToHead + "---mToHead---"
                + mToId + "---mToId---"
