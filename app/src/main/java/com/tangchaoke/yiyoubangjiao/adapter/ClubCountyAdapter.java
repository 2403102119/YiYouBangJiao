...
public class ClubCountyAdapter extends RecyclerView.Adapter<ClubCountyAdapter.MyViewHold> {

    private LayoutInflater mLayoutInflater;
    private Context mContext;
    private OnItemClickListener mOnItemClickListener;
    List<ClubProvinceInfoModel.ClubProvinceInfoListModel.ClubProvinceInfoCityListModel.ClubProvinceInfoAreaListModel> mListCounty;

    private int mSelectProvincePos = -1;//保存当前选中的position 重点!
    private int mSelectCityPos = -1;//保存当前选中的position 重点!
    private int mSelectCountyPos = -1;//保存当前选中的position 重点!

    public void setOnItemCilckLisener(OnItemClickListener mOnItemClickListener) {
        this.mOnItemClickListener = mOnItemClickListener;
    }

    public ClubCountyAdapter(int mSelectProvincePos, int mSelectCityPos, int mSelectCountyPos, Context mContext,
                             List<ClubProvinceInfoModel.ClubProvinceInfoListModel.ClubProvinceInfoCityListModel.ClubProvinceInfoAreaListModel> mListCounty,
                             OnItemClickListener mOnItemClickListener) {
        this.mSelectProvincePos = mSelectProvincePos;
        this.mSelectCityPos = mSelectCityPos;
        this.mSelectCountyPos = mSelectCountyPos;
        this.mContext = mContext;
        this.mListCounty = mListCounty;
        this.mOnItemClickListener = mOnItemClickListener;
    ...
    public void onBindViewHolder(MyViewHold holder, int position, List<Object> payloads) {
        ...
        holder.mTvClubName.setText(mListCounty.get(position).getName());

    }

    @Override
    public int getItemCount() {
        return mListCounty.size();
    }
