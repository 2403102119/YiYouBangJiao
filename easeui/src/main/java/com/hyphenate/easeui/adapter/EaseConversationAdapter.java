...
public class EaseConversationAdapter extends ArrayAdapter<EMConversation> {
    ...
    List<MessageModel.MessageModelModel> mCopyList;

    public EaseConversationAdapter(Context context, int resource,
                                   List<EMConversation> objects, List<MessageModel.MessageModelModel> mList) {
        super(context, resource, objects);
        this.context = context;
        mList = mList;
        mCopyList = new ArrayList<MessageModel.MessageModelModel>();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String mToHead = Api.PATH + mCopyList.get(position).getHead();

        String mToName = mCopyList.get(position).getNickName();

        Log.e("==消息::::", "mCopyList::::" + mCopyList.size());
        Log.e("==消息::::", "position:::" + position
                + "===nickName:::" + mCopyList.get(position).getNickName()
                + "===head:::" + mCopyList.get(position).getHead());
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
            ...
            if (!TextUtils.isEmpty(mToHead)) {
                Glide.with(getContext()).load(mToHead).into(holder.avatar);
            } else {
                Glide.with(getContext()).load(R.drawable.ic_user_z).into(holder.avatar);
            }
            if (!TextUtils.isEmpty(mToName)) {
                if (mToName.length() > 11) {
                    holder.name.setMaxLines(1);
                    holder.name.setMaxEms(6);
                    holder.name.setEllipsize(TextUtils.TruncateAt.END);
                    holder.name.setSingleLine();
                    EaseUserUtils.setUserNick(mToName, holder.name);
                } else {
                    EaseUserUtils.setUserNick(mToName, holder.name);
                }
            } else {
            ...
            Intent intent = new Intent("com.tangchaoke.yiyoubangjian.home");
            intent.putExtra("tuisong", "message");
        ...
        mIntent.putExtra("tuisong", "message_num");
